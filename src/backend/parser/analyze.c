						 ...
						 bool onmaster)
{
	...
	if(iswritable)
	{
		...
		if(distributedBy == NIL && likeDistributedBy == NIL)
		{
			...
			*policyp = createRandomDistribution();
		}
		else
		{
				...
				List *stenc)
{
	...
	if ( ! child_tab_stmt->tablespacename )
	{
		...
		if ( ! poid )
		{
			poid = RangeVarGetRelid(stmt->relation, true); /* whole partitioned table */
		}
		if ( poid )
		{
 ...
 */
GpPolicy *createRandomDistribution(void)
{
        GpPolicy*       p = NULL;
        p = (GpPolicy *) palloc(sizeof(GpPolicy));
