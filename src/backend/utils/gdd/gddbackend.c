...
#define BUILD_WAIT_GRAPH_OK 1
#define BUILD_WAIT_GRAPH_ERROR 0
...
{
	/* TODO: what is the max connection now?*/
	for (;;)
	{
		/* TODO: only run the detector when there is active transactions */
		/* TODO: allow gdd to be turned on/off at runtime via some GUC? */
...
buildWaitGraph(GddCtx *ctx)
{
	...
	PG_TRY();
	{
		...
		if (res == SPI_OK_SELECT && SPI_tuptable != NULL)
		{
			...
			/* Get a list of valid gxids, any other gxids are considered invalid */
			gxids = ListAllGxid();
			...
			for (i = 0; i < tuple_num; i++)
			{
				...
				GddCtxAddEdge(ctx, segid, waiter_xid, holder_xid, fulledge);
			}
		}
	}
	PG_CATCH();
	...
	MemoryContextSwitchTo(oldContext);
	/*
	 * SPI_finish() will delete SPI memory context and switch back to
	 * gdd memory context.
	 */
	Assert(connected);
	SPI_finish();
