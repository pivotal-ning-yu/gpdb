...
static GddEdge *gddGraphMakeEdge(GddGraph *graph, int from, int to, bool full);
...
static GddEdge *gddEdgeNew(GddVert *from, GddVert *to, bool full);
 ...
 */
void
GddCtxAddEdge(GddCtx *ctx, int segid, int from, int to, bool full)
{
	GddEdge		*edge;
	GddGraph	*graph;
	GddStat		*global;
	...
	edge = gddGraphMakeEdge(graph, from, to, full);
 ...
 */
static GddEdge *
gddGraphMakeEdge(GddGraph *graph, int from, int to, bool full)
{
	GddEdge		*edge;
	GddVert		*vfrom;
	GddVert		*vto;
	...
	/* TODO : check for duplicated edges */

	vfrom = gddGraphGetVert(graph, from);
	vto = gddGraphGetVert(graph, to);

	edge = gddEdgeNew(vfrom, vto, full);
 ...
 */
static GddEdge *
gddEdgeNew(GddVert *from, GddVert *to, bool full)
{
