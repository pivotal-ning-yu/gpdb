...
get_phase2_status_from_file(_ExpandStatus *status)
{
	...
	if (fstatus)
	{
		...
		/* the whole content of phase2 status file is dbname with no line break*/
		...
		if (dbname[0])
			...
			appendStringInfo(detail,
							 "unknown phase2 status, can not find out the correct database which contains the detailed phase2 information");
