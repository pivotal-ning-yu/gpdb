...
typedef struct GlobalTransactionData
{
	...
	XLogRecPtr	prepare_begin_lsn;		/* XLOG begging offset of prepare
										 * record */
	XLogRecPtr	prepare_lsn;	/* XLOG offset of prepare record */
	Oid			owner;			/* ID of user that executed the xact */
	TransactionId locking_xid;	/* top-level XID of backend working on xact */
	bool		valid;			/* TRUE if fully prepared */
	char		gid    [GIDSIZE];		/* The GID assigned to the prepared
										 * xact */

	int			prepareAppendOnlyIntentCount;

	/*
	 * The Append-Only Resync EOF intent count for a non-crashed prepared
	 * transaction.
	 */
...
add_recover_post_checkpoint_prepared_transactions_map_entry(TransactionId xid, XLogRecPtr * m, char *caller)
{
	...
	/*
	 * KAS should probably put out an error if found == true (i.e. it already
	 * exists).
	 */
	...
	/*
	 * If this is a new entry, we need to add the data, if we found an entry,
	 * we need to update it, so just copy our data right over the top.
	 */
...
EndPrepare(GlobalTransaction gxact)
{
	...
	/*
	 * The MirroredLock will cover BOTH mirrored writes to the pg_twophase
	 * directory and the Prepared XLOG record.
	 *
	 ...
	 * MyProc as not running our XID (which it will do immediately after this
...
FinishPreparedTransaction(const char *gid, bool isCommit, bool raiseErrorIfNotFound)
{
	...
	{
		/*
		 * Invalid XLOG record means record is corrupted. Failover is
		 * required, hopefully mirror is in healthy state.
		 */
		ereport(WARNING,
	...
	/*
	 * Although we return the end of the PersistentEndXactRec object, we
	 * really want the rounded-up aligned next buffer.	So, that is why we
	 * compute the deserialized length and calculated the next buffer with it.
	 */
	deserializeLen =
	...
	/*
	 * Use the MirroredLock to cover both the XLOG of the {COMMIT|ABORT}
	 * PREPARED record and the removal the of the two phase file from the
	 * pg_twophase directory.
	 */
	MIRRORED_LOCK;

	/*
	 * We have to lock out checkpoint start here when updating persistent
	 * relation information like Appendonly segment's committed EOF. Otherwise
	 * there might be a window betwwen the time some data is added to an
	 * appendonly segment file and its EOF updated in the persistent relation
	 * tables. If there is a checkpoint before updating the peristent tables
	 * and the system crash after the checkpoint, then during crash recovery
	 * we would not resync to the right EOFs (MPP-18261). When we use
	 * CheckpointStartLock, we make sure we already have the MirroredLock
	 * first.
	 */
	CHECKPOINT_START_LOCK;
	 ...
	 * properly later.		It is still locked by our XID so it won't go away
	 * yet.
	 *
	...
	/*
	 * Need to figure out the memory allocation and deallocationfor "buffer".
	 * For now, just let it leak.
	 */
...
{
	/*
	 * I think this is not needed with the new two phase logic. We have
	 * already attached all the prepared transactions to the checkpoint
	 * record. For now, just return from this.
	 */
	return;
...
PrescanPreparedTransactions(void)
{
	...
	while (tfXLogRecPtr != NULL)
	{
		...
		if (TransactionIdDidCommit(xid) == false && TransactionIdDidAbort(xid) == false)
		{
			...
			}					/* end for (int i = 0; i < hdr->nsubxacts;
								 * i++) */
		}						/* end if (TransactionIdDidCommit(xid) ==
								 * false && TransactionIdDidAbort(xid) ==
								 * false) */
...
SetupCheckpointPreparedTransactionList(XLogRecord * record)
{
	...
	/*
	 * Under some cercumstances, and old style checkpoint may exist (upgrade
	 * switch xlog...).
	 */

	/*
	 * Check to see if it looks like an old checkpoin, size of a checkpoint
	 * plus size of an empty DTX list or less
	 */
	if (record->xl_len <= (sizeof(CheckPoint) + TMGXACT_CHECKPOINT_BYTES(0)))
	{
		/*
		 * This is an old checkpoint (pre-removal of two phase) or a bad
		 * checkpoint record. Assume it is old and return.
		 */
		if (Debug_persistent_print)
			elog(Persistent_DebugPrintLevel(),
...
RecoverPreparedTransactions()
{
	...
	while (tfXLogRecPtr != NULL)
	{
		 ...
		 * there's no need to restore that exactly.
		 */
		 ...
		 *
		 * Note: since we don't have the PREPARE record's WAL location at hand,
		 * we leave prepare_lsn zeroes.  This means the GXACT will be fsync'd
		 * on every future checkpoint.	We assume this situation is infrequent
		 * enough that the performance cost is negligible (especially since we
		 * know the state file has already been fsynced).
		 */
		elog(Persistent_DebugPrintLevel(),
	...
	}							/* end while (xlogrecptr = (XLogRecPtr
								 * *)hash_seq_search(&hsStatus)) */
					  ...
					  PersistentEndXactRecObjects * persistentPrepareObjects)
{
	 ...
	 * Wait for synchronous replication, if required.
	 *
	 * Note that at this stage we have marked clog, but still show as running in
	 * the procarray and continue to hold locks.
	 */
					   ...
					   prepared_transaction_agg_state ** ptas, int *maxCount)
{
	...
	*maxCount = 10;
	/* Start off with at least this much room. */
