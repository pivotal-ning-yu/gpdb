...
initDistributionData(CopyState cstate, Form_pg_attribute *attr,AttrNumber num_phys_attrs)
{
	...
	if (policy)
		...
		p_nattrs = 0;
	/* Create hash API reference */
	cdbHash = makeCdbHash(total_segs, HASH_FNV_1);
		...
		*(stop-1) = endchar;
		...
		if (scan_end == NULL)
		{
			...
			if (attr_pre_len == cstate->null_print_len
				&&
				strncmp(cstate->line_buf.data + cstate->line_buf.len - attr_pre_len - (cstate->eol_type == EOL_CRLF ? 2 : 1), cstate->null_print, attr_pre_len)
				== 0)
				nulls[m] = true;
			else
				nulls[m] = false;
