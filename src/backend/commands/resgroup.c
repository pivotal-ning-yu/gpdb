...
static float floatFromText(const text * text);
...
{
	char value[64];
	Relation resgroup_capability_rel = heap_open(ResGroupCapabilityRelationId, ExclusiveLock);
 ...
 *
 * @return the option type or UNKNOWN if the name is knknown
 */
static int
					 ...
					 ResourceGroupOptions *options)
{
	...
	while (HeapTupleIsValid(tuple = systable_getnext(sscan)))
	{
		...
		{
			totalCpu += floatFromText(&resgCapability->value);
			if (totalCpu > 1.0)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
		...
		{
			totalMem += floatFromText(&resgCapability->value);
			if (totalMem > 1.0)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
