...
bool
resizeGang_async(Gang *gang, int size)
{
	...
	ELOG_DISPATCHER_DEBUG("resizeGang type = %d, gang_id = %d, size = %d->%d",
						  gang->type, gang->gang_id, gang->size, size);
	...
	resizeGangDefinition(gang, size);
...
create_gang_retry:
	...
	successful_connections = oldsize;
	in_recovery_mode_count = 0;
	retry = false;

	Assert(gang->perGangContext != NULL);
	MemoryContextSwitchTo(gang->perGangContext);
	...
	PG_CATCH();
	{
		...
		if (FtsTestSegmentDBIsDown(gang->db_descriptors, size))
		{
		...
		if (gang->type == GANGTYPE_PRIMARY_WRITER)
		{
			DisconnectAndDestroyAllGangs(true);
			CheckForResetSession();
		}
