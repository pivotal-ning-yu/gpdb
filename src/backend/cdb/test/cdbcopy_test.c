...
test__processCopyEndResults_handles_int64_aggregation(void **state)
{
	...
	mock_result->numCompleted = INT64_MAX;
	...
	expect_any(process_aotupcounts, parts);
	expect_any(process_aotupcounts, ht);
	expect_any(process_aotupcounts, aotupcounts);
	expect_any(process_aotupcounts, naotupcounts);
	will_return(process_aotupcounts, 0);
	...
	assert_true(total_rows_completed == INT64_MAX);
