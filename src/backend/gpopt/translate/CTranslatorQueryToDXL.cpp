	...
	)
{
	...
	{		
		...
		{
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature, GPOS_WSZ_LIT("CTAS. Set optimizer_enable_ctas to on to enable CTAS with PQO"));
		}
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	const ULONG num_non_dropped_cols = md_rel->NonDroppedColsCount() - md_rel->SystemColumnsCount();
	...
	)
{
	...
	CHAR *value = gpdb::DefGetString(def_elem, &needs_free);
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorQueryToDXL::IsLeadWindowSpec
//
//	@doc:
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorQueryToDXL::IsLagWindowSpec
//
//	@doc:
	...
	const
{
	...
	if (NULL == dxl_offset)
	{
		...
		dxl_lead_edge->AddChild(CTranslatorUtils::CreateDXLProjElemFromInt4Const(m_mp, m_md_accessor, 1 /*iVal*/));
		dxl_trail_edge->AddChild(CTranslatorUtils::CreateDXLProjElemFromInt4Const(m_mp, m_md_accessor, 1 /*iVal*/));
	}
	else
	{
	...
	const
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		BOOL is_lead_func = IsLeadWindowSpec(dxlop);
		BOOL is_lag = IsLagWindowSpec(dxlop);
	...
	)
{
	...
	ForEach (lc, window_clause)
	{
		...
		ULongPtrArray *part_columns = TranslatePartColumns(ws->partition, sort_col_attno_to_colid_mapping);
		...
		if (NULL != ws->name)
		{
			CWStringDynamic *alias_str = CDXLUtils::CreateDynamicStringFromCharArray(m_mp, ws->name);
			mdname = GPOS_NEW(m_mp) CMDName(m_mp, alias_str);
			GPOS_DELETE(alias_str);
		}

		if (0 < gpdb::ListLength(ws->order))
		{
			// create a sorting col list
			sort_col_list_dxl = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarSortColList(m_mp));

			CDXLNodeArray *dxl_sort_cols = TranslateSortColumnsToDXL(ws->order, sort_col_attno_to_colid_mapping);
		...
		if (NULL != ws->frame)
		{
			window_frame = m_scalar_translator->GetWindowFrame((Expr *)ws->frame, m_var_to_colid_map,project_list_dxlnode_node,&m_has_distributed_tables);
		}
