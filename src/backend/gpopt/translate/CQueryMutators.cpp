	...
	)
{
	...
	ForEach (lc, query->targetList)
	{
		...
		if (!IsA(target_entry->expr, PercentileExpr) &&
	...
	)
{
	...
	if (IsA(node, Aggref))
	{
		...
		TargetEntry *target_entry = GetTargetEntryForAggOrPercentileExpr(context->m_mp, context->m_md_accessor, (Node *) aggref, attno);
	...
	if (IsA(node, PercentileExpr) || IsA(node, GroupingFunc))
	{
		...
		const ULONG attno = gpdb::ListLength(context->m_groupby_target_list) + 1;
		TargetEntry *target_entry = GetTargetEntryForAggOrPercentileExpr(context->m_mp, context->m_md_accessor, pnodeCopy, attno);
...
//---------------------------------------------------------------------------
//	@function:
//		CQueryMutators::GetTargetEntryForAggOrPercentileExpr
//
//	@doc:
	...
	)
{
	...
	const ULONG attno = gpdb::ListLength(context->m_groupby_target_list) + 1;
	TargetEntry *target_entry = GetTargetEntryForAggOrPercentileExpr(context->m_mp, context->m_md_accessor, (Node *) node, attno);
	context->m_groupby_target_list = gpdb::LAppend(context->m_groupby_target_list, target_entry);
	...
	)
{
	...
	if (IsA(node, WindowRef))
	{
		...
		WindowRef *windowref = (WindowRef*) gpdb::CopyObject(node);

		// get the function name and add it to the target list
		CMDIdGPDB *mdid_func = GPOS_NEW(context->m_mp) CMDIdGPDB(windowref->winfnoid);
