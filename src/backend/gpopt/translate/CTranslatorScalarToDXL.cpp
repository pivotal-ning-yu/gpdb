...
{
	GPOS_ASSERT_IMP(!is_query_mode, pplstmt);
}
	...
	)
{
	static const STranslatorElem translators[] =
	{
		...
		{T_WindowRef, &CTranslatorScalarToDXL::CreateScalarWindowSpec},
	...
	)
{
	...
	const WindowFrame *wf = (WindowFrame *) expr;
	...
	EdxlFrameBoundary leading_boundary = Edxlfb(wf->lead->kind, wf->lead->val);
	EdxlFrameBoundary trailing_boundary = Edxlfb(wf->trail->kind, wf->trail->val);;
	...
	if (NULL != wf->lead->val)
	{
		lead_edge->AddChild(TranslateWindowFrameEdgeToDXL(wf->lead->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}

	if (NULL != wf->trail->val)
	{
		trail_edge->AddChild(TranslateWindowFrameEdgeToDXL(wf->trail->val, var_colid_mapping, new_scalar_proj_list, has_distributed_tables));
	}
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorScalarToDXL::CreateScalarWindowSpec
//
//	@doc:
