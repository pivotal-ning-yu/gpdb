	...
	)
{
	...
	OID op_family_oid = rel->rd_indclass->values[attno - 1];
	...
	ForEach (grouping_set, grouping_clause->groupsets)
	{
		...
		else if (IsA(node, List))
		{
			...
			ForEach (group_clause, list)
			{
				...
				GroupClause *group_clause = (GroupClause *) group_clause_node;
	...
	)
{
	...
	ForEach (lc, group_elems)
	{
		...
		ULONG group_ref = ((GroupClause *) elem_node)->tleSortGroupRef;
		bs->ExchangeSet(group_ref);
		
		UpdateGrpColMapping(mp, group_col_pos, group_cols, group_ref);
	}
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorUtils::CreateDXLProjElemFromInt4Const
//
//	@doc:
...
{
	GPOS_ASSERT(NULL != mp);
	const IMDTypeInt4 *md_type_int4 = md_accessor->PtMDType<IMDTypeInt4>();
	md_type_int4->MDId()->AddRef();

	CDXLDatumInt4 *datum_dxl = GPOS_NEW(mp) CDXLDatumInt4(mp, md_type_int4->MDId(), false /*fConstNull*/, val);
	...
	)
{
	...
	ForEach (group_clause_cell, group_clause)
	{
		...
		GroupClause *group_clause = (GroupClause *) group_clause_node;
		if (sort_clause->tleSortGroupRef == group_clause->tleSortGroupRef)
		{
			return true;
		}
	}
