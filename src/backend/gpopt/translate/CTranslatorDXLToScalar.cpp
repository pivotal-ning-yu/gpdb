		...
		)
{
	...
	pcoerce->funcresulttype = CMDIdGPDB::CastMdid(pdxlop->GetResultTypeMdId())->Oid();
	pcoerce->funcretset = false;
	pcoerce->funcformat = (CoercionForm)  pdxlop->GetDXLCoercionForm();
	pcoerce->args = TranslateScalarChildren(NULL, pdxlnCoerce, pmapcidvar);
	...
	)
{
	...
	WindowRef *windowref = MakeNode(WindowRef);
	windowref->winfnoid = CMDIdGPDB::CastMdid(dxlop->FuncMdId())->Oid();
	windowref->windistinct = dxlop->IsDistinct();
	windowref->winlevelsup = 0;
	windowref->location = -1;
	windowref->winlevel = 0;
	windowref->winspec = dxlop->GetWindSpecPos();
	windowref->restype = CMDIdGPDB::CastMdid(dxlop->ReturnTypeMdId())->Oid();
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		{
			windowref->winstage = (WinStage) elem[0];
			break;
		}
	}

	// translate the arguments of the window function
	windowref->args = TranslateScalarChildren(windowref->args, scalar_winref_node, colid_var);

	return (Expr *) windowref;
}
	...
	)
{
	...
	Plan *plan = ((CMappingColIdVarPlStmt*) colid_var)->Pplan();
	...
	Plan *plan_child = dxl_to_plstmt_translator.TranslateDXLOperatorToPlan(child_dxl, &subplan_translate_ctxt, plan, prev_siblings_ctxt_arr);
	...
	)
{
	...
	return (Expr *) gpdb::EvalConstExpressions(expr);
}
