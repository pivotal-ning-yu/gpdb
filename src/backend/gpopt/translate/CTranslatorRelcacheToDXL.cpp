	...
	)
{
	...
	mdid_index->AddRef();
	IMdIdArray *op_classes_mdids = RetrieveIndexOpClasses(mp, mdid_index);
	...
	)
{
	...
	IMdIdArray *pdrgpmdidOpClasses = RetrieveIndexOpClasses(mp, mdid_index);
	...
	)
{
	...
	CMDIdGPDB *mdid_count = GPOS_NEW(mp) CMDIdGPDB(GPDB_COUNT_AGG_OID);
	...
	)
{
	...
	// TODO: frahman 07/01/2016: remove hard coded function properties map when catalog is fixed to include correct properties.
	// Lookup function properties first in patchy catalog bypass
	if (GPDB_PG_GET_PART_RULE_DEF_1 == func_oid || GPDB_PG_GET_PART_RULE_DEF_2 == func_oid)
	{
		*stability = IMDFunction::EfsStable;
		*access = IMDFunction::EfdaReadsSQLData;
		*is_strict = true;
		*returns_set = true;
	...
	)
{
	...
	BOOL cast_exists = gpdb::GetCastFunc(src_oid, dest_oid, &is_binary_coercible, &cast_fn_oid);
	...
	)
{
	...
	*part_types = GPOS_NEW(mp) CharPtrArray(mp);
	...
	PartitionNode *pn = gpdb::GetParts(oid, 0 /*level*/, 0 /*parent*/, false /*inctemplate*/, mcxt, true /*includesubparts*/);
	GPOS_ASSERT(NULL != pn);
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorRelcacheToDXL::RetrieveIndexOpClasses
//
//	@doc:
...
{
	List *op_classes = gpdb::GetIndexOpClasses(CMDIdGPDB::CastMdid(mdid_index)->Oid());
	IMdIdArray *input_col_mdids = GPOS_NEW(mp) IMdIdArray(mp);
	
	ListCell *lc = NULL;
	
	ForEach(lc, op_classes)
	{
		OID op_class_oid = lfirst_oid(lc);
		input_col_mdids->Append(GPOS_NEW(mp) CMDIdGPDB(op_class_oid));
	}
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorRelcacheToDXL::RetrieveScOpOpClasses
//
//	@doc:
...
{
	List *op_classes = gpdb::GetOpClassesForScOp(CMDIdGPDB::CastMdid(mdid_scalar_op)->Oid());
	IMdIdArray *input_col_mdids = GPOS_NEW(mp) IMdIdArray(mp);
