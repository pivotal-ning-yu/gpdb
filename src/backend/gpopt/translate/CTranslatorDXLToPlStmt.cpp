	...
	)
{
	...
	Plan *plan = TranslateDXLOperatorToPlan(dxlnode, &dxl_translate_ctxt, NULL, ctxt_translation_prev_siblings);
	...
	)
{
	...
	return (this->* dxlnode_to_logical_funct)(dxlnode, output_context, parent_plan, ctxt_translation_prev_siblings);
}
...
CTranslatorDXLToPlStmt::SetInitPlanVariables(PlannedStmt* planned_stmt)
{
	...
	planned_stmt->planTree->nParamExec = m_dxl_to_plstmt_context->GetCurrentParamId();
	planned_stmt->nCrossLevelParams = m_dxl_to_plstmt_context->GetCurrentParamId();
	...
	)
{
	...
	ForEach (plcLocation, extentry->locations)
	{
		...
		Uri *pUri = gpdb::ParseExternalTblUri(szUri);
	...
	)
{
	...
	const ULONG ulLocations = gpdb::ListLength(extentry->locations);
	...
	)
{
	...
	Value *pvOnClause = (Value *) gpdb::ListNth(extentry->locations, 0);
	...
	)
{
	...
	GPOS_ASSERT(0 < gpdb::ListLength(extentry->locations));
	
	CHAR *szFirstUri = NULL;
	Uri *pUri = NULL;
	if (!fUsingExecute)
	{
		szFirstUri = ((Value *) gpdb::ListNth(extentry->locations, 0))->val.str;
		pUri = gpdb::ParseExternalTblUri(szFirstUri);
	}
	...
	)
{
	...
	const ULONG num_rel_columns = md_rel->ColumnCount() - md_rel->SystemColumnsCount();
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(dxl_table_descr, NULL /*index_descr_dxl*/, num_rel_columns, index, &base_table_context);
	...
	if (IMDRelation::ErelstorageExternal == md_rel->RetrieveRelStorageType())
	{
		...
		IMDId *mdid_FmtErrTbl = md_rel_ext->GetFormatErrTableMdid();
		if (NULL == mdid_FmtErrTbl)
		{
			ext_scan->fmterrtbl = InvalidOid;
		}
		else
		{
			ext_scan->fmterrtbl = CMDIdGPDB::CastMdid(mdid_FmtErrTbl)->Oid();
		}
	...
	plan->plan_parent_node_id = IPlanId(parent_plan);
	plan->nMotionNodes = 0;
	...
	)
{
	...
	return TranslateDXLIndexScan(index_scan_dxlnode, physical_idx_scan_dxlop, output_context, parent_plan, false /*is_index_only_scan*/, ctxt_translation_prev_siblings);
}
	...
	)
{
	...
	const ULONG num_rel_columns = md_rel->ColumnCount() - md_rel->SystemColumnsCount();
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(physical_idx_scan_dxlop->GetDXLTableDescr(), index_descr_dxl, num_rel_columns, index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	m_dxl_to_plstmt_context->AddRTE(rte);
	...
	// translate proj list
	plan->targetlist = TranslateDXLProjList(project_list_dxlnode, &base_table_context, NULL /*child_contexts*/, output_context, plan);
	...
	)
{
	...
	CMappingColIdVarPlStmt colid_var_mapping(m_mp, base_table_context, ctxt_translation_prev_siblings, output_context, m_dxl_to_plstmt_context, parent_plan);
	...
	)
{
	...
	// build colid->var mapping
	CMappingColIdVarPlStmt colid_var_mapping(m_mp, NULL /*base_table_context*/, child_contexts, output_context, m_dxl_to_plstmt_context, parent_plan);
	...
	)
{
	...
	Plan *left_plan = TranslateDXLOperatorToPlan(child_plan_dxlnode, &left_dxl_translate_ctxt, plan, ctxt_translation_prev_siblings);
	...
	{
		CMappingColIdVarPlStmt colid_var_mapping(m_mp, NULL, child_contexts, output_context, m_dxl_to_plstmt_context, plan);
	...
	{
		CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt(m_mp, NULL, child_contexts, output_context, m_dxl_to_plstmt_context, plan);
	...
	)
{
	...
	Plan *left_plan = TranslateDXLOperatorToPlan(left_tree_dxlnode, &left_dxl_translate_ctxt, plan, ctxt_translation_prev_siblings);
	...
	Plan *right_plan = (Plan*) TranslateDXLHash(right_tree_dxlnode, &right_dxl_translate_ctxt, plan, translation_context_arr_with_siblings);
	...
	)
{
	...
	RangeTblEntry *rte = TranslateDXLTvfToRangeTblEntry(tvf_dxlnode, output_context, &base_table_context, plan);
	GPOS_ASSERT(NULL != rte);
	...
	ForEach (lc_target_entry, target_list)
	{
		...
		rte->funccoltypes = gpdb::LAppendOid(rte->funccoltypes, oid_type);
		rte->funccoltypmods = gpdb::LAppendInt(rte->funccoltypmods, typ_mod);
	}
	...
	)
{
	...
	CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt(m_mp, base_table_context, NULL, output_context, m_dxl_to_plstmt_context, parent_plan);
	...
	)
{
	...
	if (dxl_nlj->IsIndexNLJ())
	{
		...
		left_plan = TranslateDXLOperatorToPlan(left_tree_dxlnode, &left_dxl_translate_ctxt, plan, ctxt_translation_prev_siblings);

		translation_context_arr_with_siblings->Append(&left_dxl_translate_ctxt);
		translation_context_arr_with_siblings->AppendArray(ctxt_translation_prev_siblings);

		 // translate right child after left child translation is complete
		right_plan = TranslateDXLOperatorToPlan(right_tree_dxlnode, &right_dxl_translate_ctxt, plan, translation_context_arr_with_siblings);
	}
	else
	{
		...
		right_plan = TranslateDXLOperatorToPlan(right_tree_dxlnode, &right_dxl_translate_ctxt, plan, ctxt_translation_prev_siblings);
		...
		left_plan = TranslateDXLOperatorToPlan(left_tree_dxlnode, &left_dxl_translate_ctxt, plan, translation_context_arr_with_siblings);
	}
	CDXLTranslationContextArray *child_contexts = GPOS_NEW(m_mp) CDXLTranslationContextArray(m_mp);
	child_contexts->Append(&left_dxl_translate_ctxt);
	...
	)
{
	...
	Plan *right_plan = TranslateDXLOperatorToPlan(right_tree_dxlnode, &right_dxl_translate_ctxt, plan, translation_context_arr_with_siblings);
	...
	)
{
	...
	Plan *left_plan = TranslateDXLOperatorToPlan(dxlnode, &dxl_translate_ctxt, plan, ctxt_translation_prev_siblings);
	...
	)
{
	...
	{
		return TranslateDXLRedistributeMotionToResultHashFilters(motion_dxlnode, output_context, parent_plan, ctxt_translation_prev_siblings);
	}
	
	return TranslateDXLMotion(motion_dxlnode, output_context, parent_plan, ctxt_translation_prev_siblings);
}
	...
	)
{
	...
	Plan *child_plan = TranslateDXLOperatorToPlan(child_dxlnode, &child_context, plan, ctxt_translation_prev_siblings);
	...
	if (0 < num_sort_cols)
	{
		...
		TranslateSortCols(sort_col_list_dxl, output_context, motion->sortColIdx, motion->sortOperators);
	}
	else
	{
	...
	)
{
	...
	{
		WindowKey *window_key = MakeNode(WindowKey);

		// translate the sorting columns used in the window key
		const CDXLWindowKey *window_key_dxl = window_dxlop->GetDXLWindowKeyAt(0);
		const CDXLNode *sort_col_list_dxlnode = window_key_dxl->GetSortColListDXL();
		
		const ULONG num_of_cols = sort_col_list_dxlnode->Arity();
		window_key->numSortCols = num_of_cols;
		window_key->sortColIdx = (AttrNumber *) gpdb::GPDBAlloc(num_of_cols * sizeof(AttrNumber));
		window_key->sortOperators = (Oid *) gpdb::GPDBAlloc(num_of_cols * sizeof(Oid));
		TranslateSortCols(sort_col_list_dxlnode, &child_context, window_key->sortColIdx, window_key->sortOperators);

		// translate the window frame specified in the window key
		window_key->frame = NULL;
		if (NULL != window_key_dxl->GetWindowFrame())
		{
			window_key->frame = Pwindowframe(window_key_dxl->GetWindowFrame(), &child_context, output_context, plan);
		}
		window->windowKeys = gpdb::LAppend(window->windowKeys, window_key);
	}
	...
	)
{
	...
	{
		pwindowframe->lead->val = (Node*) m_translator_dxl_to_scalar->TranslateDXLToScalar((*pdxlnLead)[0], &mapcidvarplstmt);
	}
	...
	{
		pwindowframe->trail->val = (Node*) m_translator_dxl_to_scalar->TranslateDXLToScalar((*pdxlnTrail)[0], &mapcidvarplstmt);
	}
	...
	)
{
	...
	CDXLNode *limit_count_dxlnode = (*sort_dxlnode)[EdxlsortIndexLimitCount];
	CDXLNode *limit_offset_dxlnode = (*sort_dxlnode)[EdxlsortIndexLimitOffset];
	...
	TranslateSortCols(sort_col_list_dxl, &child_context, sort->sortColIdx, sort->sortOperators);

	// translate limit information
	if (0 < limit_count_dxlnode->Arity())
	{
		GPOS_ASSERT(1 == limit_count_dxlnode->Arity());
		const CDXLNode *limit_count_expr_dxlnode = (*limit_count_dxlnode)[0];
		CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt
																(
																m_mp,
																NULL,
		...
		sort->limitCount = (Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(limit_count_expr_dxlnode, &colid_var_mapping);
	}

	if (0 < limit_offset_dxlnode->Arity())
	{
		GPOS_ASSERT(1 == limit_offset_dxlnode->Arity());
		const CDXLNode *limit_offset_expr_dxlnode = (*limit_offset_dxlnode)[0];
		...
		sort->limitOffset = (Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(limit_offset_expr_dxlnode, &colid_var_mapping);
	}
	SetParamIds(plan);
	...
	)
{
	...
	// translate proj list
	plan->targetlist = TranslateDXLProjList(project_list_dxlnode, NULL /*base_table_context*/, child_contexts, output_context, plan);

	// translate filter lists
	GPOS_ASSERT(eq_filters_dxlnode->Arity() == num_of_levels);
	partition_selector->levelEqExpressions = TranslateDXLFilterList(eq_filters_dxlnode, NULL /*base_table_context*/, child_contexts, output_context, plan);

	GPOS_ASSERT(filters_dxlnode->Arity() == num_of_levels);
	partition_selector->levelExpressions = TranslateDXLFilterList(filters_dxlnode, NULL /*base_table_context*/, child_contexts, output_context, plan);

	//translate residual filter
	CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt(m_mp, NULL /*base_table_context*/, child_contexts, output_context, m_dxl_to_plstmt_context, plan);
	...
	)
{
	...
	CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt(m_mp, base_table_context, child_contexts, output_context, m_dxl_to_plstmt_context, parent_plan);
	...
	)
{
	...
	CDXLPhysicalAppend *append_dxl_node = CDXLPhysicalAppend::Cast(append_dxlnode->GetOperator());

	append->isTarget = append_dxl_node->IsUsedInUpdDel();
	append->isZapped = append_dxl_node->IsZapped();
	...
	)
{
	...
	Plan *plan = &(shared_input_scan->plan);
	plan->plan_node_id = m_dxl_to_plstmt_context->GetNextPlanId();
	plan->plan_parent_node_id = IPlanId(parent_plan);
	...
	if (!IsA(child_plan, Material) && !IsA(child_plan, Sort))
	{
		...
		materialize_plan->plan_parent_node_id = plan->plan_node_id ;
	...
	)
{
	...
	ForEach (lc_sh_scan_cte_consumer, shared_scan_cte_consumer_list)
	{
		...
		if (NULL == (share_input_scan_consumer->plan).flow)
		{
			(share_input_scan_consumer->plan).flow = (Flow *) gpdb::CopyObject(flow);
		}
	}
}
	...
	)
{
	...
	{
		ShareInputScan *share_input_scan_consumer = (ShareInputScan *) lfirst(lc_sh_scan_cte_consumer);
		Flow *flow_cte = (share_input_scan_consumer->plan).flow;
		if (NULL != flow_cte)
		{
			if (NULL == flow)
			{
	...
	)
{
	...
	Plan *plan = &(share_input_scan_cte_consumer->plan);
	plan->plan_node_id = m_dxl_to_plstmt_context->GetNextPlanId();
	plan->plan_parent_node_id = IPlanId(parent_plan);
	...
	)
{
	...
	Plan *last_child_plan = TranslateDXLOperatorToPlan(last_child_dxlnode, &child_context, plan, ctxt_translation_prev_siblings);
	...
	)
{
	...
	const IMDRelation *md_rel = m_md_accessor->RetrieveRel(dyn_tbl_scan_dxlop->GetDXLTableDescr()->MDId());
	const ULONG num_rel_columns = md_rel->ColumnCount() - md_rel->SystemColumnsCount();
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(dyn_tbl_scan_dxlop->GetDXLTableDescr(), NULL /*index_descr_dxl*/, num_rel_columns, index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	...
	)
{
	...
	const ULONG num_rel_columns = md_rel->ColumnCount() - md_rel->SystemColumnsCount();
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(dyn_index_scan_dxlop->GetDXLTableDescr(), NULL /*index_descr_dxl*/, num_rel_columns, index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	m_dxl_to_plstmt_context->AddRTE(rte);
	...
	)
{
	...
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(table_descr, NULL /*index_descr_dxl*/, num_rel_columns, index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	...
	)
{
	...
	for (ULONG ul = last_attno + 1; ul <= num_rel_columns; ul++)
	{
		Value *val_dropped_colname = gpdb::MakeStringValue("");
		alias->colnames = gpdb::LAppend(alias->colnames, val_dropped_colname);
	}
	...
	)
{
	...
	return TranslateDXLScCondToQual(filter_cond_dxlnode, base_table_context, child_contexts, output_context, parent_plan);
}
	...
	)
{
	...
	Plan *plan = TranslateDXLOperatorToPlan(child_dxlnode, &child_context, parent_plan, ctxt_translation_prev_siblings);
	...
	result_plan->plan_parent_node_id = IPlanId(parent_plan);
	result_plan->nMotionNodes = plan->nMotionNodes;
	result_plan->lefttree = plan;
	plan->plan_parent_node_id = IPlanId(result_plan);
	...
	)
{
	...
	GPOS_ASSERT(InvalidOid != index_oid);
	Plan *parent_plan = (Plan *) bitmap_tbl_scan;
	...
	)
{
	...
	RangeTblEntry *rte = TranslateDXLValueScanToRangeTblEntry(value_scan_dxlnode, output_context, &base_table_context, plan);
	GPOS_ASSERT(NULL != rte);
