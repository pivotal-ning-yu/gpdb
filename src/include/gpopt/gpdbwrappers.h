...
namespace gpdb {
	...
	bool GetCastFunc(Oid src_oid, Oid dest_oid, bool *is_binary_coercible, Oid *cast_fn_oid);

	// get type of operator
	uint GetComparisonType(Oid op_oid, Oid left_oid, Oid right_oid);

	// get scalar comparison between given types
	Oid GetComparisonOperator(Oid left_oid, Oid right_oid, uint cmpt);
	...
	// parts of a partitioned table
	PartitionNode *GetParts(Oid relid, int2 level, Oid parent, bool inctemplate, MemoryContext mcxt, bool includesubparts);
...
gpdb::GpdbEreportImpl(xerrcode, severitylevel, xerrmsg, xerrhint , __FILE__, __LINE__, PG_FUNCNAME_MACRO)
	...
	Uri *ParseExternalTblUri(const char *uri);
	...
	char *DefGetString(DefElem *defelem, bool *needs_free);
	...
	Node *EvalConstExpressions(ArrayExpr *arrayexpr);
