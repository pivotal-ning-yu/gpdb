...
namespace gpdxl
{
	...
	class CTranslatorQueryToDXL
	{
		...
		private:
			...
			CDXLNode *TranslateExprToDXLProject(Expr *expr, CHAR *alias_name, BOOL insist_new_colids = false);
			...
			static
			BOOL IsLeadWindowSpec(CDXLOperator *dxlop);

			// check if given operator is lag() window function
			static
			BOOL IsLagWindowSpec(CDXLOperator *dxlop);
