...
DROP TABLE IF EXISTS bfv_statistics_foo;
...
select check_row_count('explain select * from bfv_statistics_foo where a is not null and b >= 1;', 3);
drop table bfv_statistics_foo;

DROP TABLE IF EXISTS bfv_statistics_foo2;
...
select check_row_count('explain select a from bfv_statistics_foo2 where a > 1 order by a;', 14);
...
select check_row_count('explain select a from bfv_statistics_foo2 where a > 1 order by a;', 8);
...
drop table if exists bfv_statistics_foo3;
create table bfv_statistics_foo3(a int);

select * from gp_toolkit.gp_stats_missing where smischema = 'public' AND  smitable = 'bfv_statistics_foo3';
...
insert into bfv_statistics_foo3 select i from generate_series(1,99) i;
insert into bfv_statistics_foo3 values (NULL);
analyze bfv_statistics_foo3;

select stanullfrac, stadistinct, stanumbers1 from pg_statistic where starelid='bfv_statistics_foo3'::regclass and staattnum=1;

select check_row_count('explain select a from bfv_statistics_foo3 where a > 888;', 1);
...
drop table if exists bfv_statistics_foo_real;
...
drop table if exists bfv_statistics_foo_numeric;
