...
class UpgradeTestCase(MPPTestCase):
    ...
    def copy_binaries(self):
        """
        Set environment variable GPDB_COPY_BIN_FROM to location of Greenplum installer

        Note: QAUtils is not needed at the moment so it is not considered here.
        """

        target_dir = "%s/gpdb_old/" % self.upgrade_home
        gpdb_copy_bin_from = os.environ.get('GPDB_COPY_BIN_FROM', '/tmp')

        logger.debug("Copying binaries from %s over to target directory %s" % (gpdb_copy_bin_from, target_dir))
        shutil.copytree(gpdb_copy_bin_from, target_dir)
        return target_dir

    # When providing an existing set of installed binaries, both old and new locations should be provided
    def setup_upgrade(self, old_gpdb_path=None, new_gpdb_path=None, mirror_enabled=False,
        ...
        (old_gpdb_path, new_gpdb_path) = self.get_gpdbpath_info()
