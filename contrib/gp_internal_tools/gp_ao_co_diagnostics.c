...
/*
 * Introduced during a cherry-pick where IS_QUERY_DISPATCHER()
 * was a macro with this definition.
 */
static bool is_query_dispatcher() {
...
#define NUM_ATTRS 8
		...
		TupleDescInitEntry(tupdesc, (AttrNumber) 6, "state",
						   INT2OID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber) 7, "is_full",
						   BOOLOID, -1, 0);
		TupleDescInitEntry(tupdesc, (AttrNumber) 8, "aborted",
						   BOOLOID, -1, 0);
	...
	if (context->segno < MAX_AOREL_CONCURRENCY)
	{
		...
		values[6] = BoolGetDatum(aosegfile->isfull);
		values[7] = BoolGetDatum(aosegfile->aborted);
