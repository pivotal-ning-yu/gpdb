...
class gpload:
  ...
  # Fast path to find out whether we have an existing external table in the
    ...
    #
    def get_fast_match_exttable_query(self, formatType, formatOpts, limitStr, errorTableOid, schemaName, log_errors):
            ...
            if self.staging_table:
                ...
                if self.fast_match:
                    sql = self.get_fast_match_exttable_query(formatType, self.formatOpts,
                        limitStr, self.error_table_oid, self.extSchemaName, self.log_errors)
                else:
    ...
    def count_errors(self):
        ...
        if (self.error_table or self.log_errors) and not self.options.D:
            ...
            if not self.reuse_tables:
                ...
                if self.error_table: # as we won't support error table any more
    ...
    def do_method(self):
        ...
        if preload:
            ...
            self.staging_table = self.getconfig('gpload:preload:staging_table', unicode, None)
            if self.error_table and self.staging_table:
                self.control_file_error("gpload:preload:staging_table and gpload:input:error_table are not allowd to use together.")
        if truncate == True:
            if method=='insert':
