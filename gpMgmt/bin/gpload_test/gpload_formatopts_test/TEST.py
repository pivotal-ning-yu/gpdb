...
class GPLoad_FormatOpts_TestCase(unittest.TestCase):
    ...
    def test_22_gpload_error_count(self):
        ...
        self.doTest(22)
    def test_23_gpload_ext_staging_table(self):
        "23  gpload reuse ext_staging_table if it is configured"
        file = mkpath('setup.sql')
        runfile(file)
        ...
        self.doTest(23)
    def test_24_gpload_ext_staging_table_with_externalschema(self):
        "24  gpload reuse ext_staging_table if it is configured with externalschema"
        file = mkpath('setup.sql')
        runfile(file)
        ...
        self.doTest(24)
    def test_25_gpload_ext_staging_table_with_externalschema(self):
        "25  gpload reuse ext_staging_table if it is configured with externalschema"
        file = mkpath('setup.sql')
        runfile(file)
        ...
        self.doTest(25)
    def test_26_gpload_ext_staging_table_with_dot(self):
        "26  gpload reuse ext_staging_table if it is configured with dot"
        file = mkpath('setup.sql')
        runfile(file)
        ...
        self.doTest(26)
    def test_27_gpload_ext_staging_table_with_dot(self):
        "27  gpload reuse ext_staging_table if it is configured with dot"
        file = mkpath('setup.sql')
        runfile(file)
    ...
    def test_29_gpload_reuse_table_update_mode_with_fast_match(self):
        "29  gpload update mode with fast match"
        drop_tables()
        copy_data('external_file_04.txt','data_file.txt')
        write_config_file(mode='update',reuse_flag='true',fast_match='true',file='data_file.txt')
        self.doTest(29)

    def test_30_gpload_reuse_table_update_mode_with_fast_match_and_different_columns_number(self):
        "30 gpload update mode with fast match and differenct columns number) "
        psql_run(cmd="ALTER TABLE texttable ADD column n8 text",dbname='reuse_gptest')
        copy_data('external_file_08.txt','data_file.txt')
        write_config_file(mode='update',reuse_flag='true',fast_match='true',file='data_file.txt')
        self.doTest(30)

    def test_31_gpload_update_mode_without_reuse_table_with_fast_match(self):
        "31  gpload update mode when reuse table is false and fast match is true"
        drop_tables()
        copy_data('external_file_08.txt','data_file.txt')
        write_config_file(mode='update',reuse_flag='false',fast_match='true',file='data_file.txt')
        self.doTest(31)

    def test_32_gpload_reuse_table_merge_mode_with_fast_match_and_external_schema(self):
        "32  gpload update mode with fast match and external schema"
        file = mkpath('setup.sql')
        runfile(file)
        copy_data('external_file_04.txt','data_file.txt')
